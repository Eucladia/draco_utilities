use crate::polyfills::globals::{utils, UriError};
...
pub fn decode_uri(bytes: &[u8], decoded: &mut Vec<u16>) -> Result<(), UriError> {
  const RESERVED: &[u8] = b"#$&+,/:;=?@";
...
pub fn decode_uri_component(bytes: &[u8], decoded: &mut Vec<u16>) -> Result<(), UriError> {
  ...
  while idx < bytes.len() {
    ...
    if byte == b'%' {
      ...
      for i in 0..bytes_needed / CHUNKS {
        ...
        let upcoming_byte = utils::hex_value(first, second);
      ...
      if total < UTF8_SURROGATE {
        if reserved.iter().any(|&x| x as u32 == total) {
          decoded.extend_from_slice(&[
            b'%' as u16,
            unsafe { *bytes.get_unchecked(idx - 1) } as u16,
            unsafe { *bytes.get_unchecked(idx) } as u16,
          ]);
        } else {
          decoded.push(total as u16);
        }
      } else {
        let low = ((total - UTF8_SURROGATE) & 0x3FF) + 0xDC00;
        let high = (((total - UTF8_SURROGATE) >> RESERVED_OCTETS_BITS) & 0x3FF) + 0xD800;

        decoded.extend_from_slice(&[high as u16, low as u16]);
      }
    } else {
      decoded.push(byte as u16);
    }
